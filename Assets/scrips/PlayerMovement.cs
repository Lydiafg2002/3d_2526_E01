...
public class PlayerMovement : MonoBehaviour
{
    ...
    public void QuickSpin(int dir)
    {
        if (!DOTween.IsTweening(model))
        {
            ...
            //gehitu buelta ematen gertatzen dena
            //artikula.Play()
            //DEsaktivatu colliderra
    ...
    public void Boost(bool state)
    {
        ...
        float originDistorsion = state ? 0 : -6f;
        float endDistorsion = state ? -6f : 0;
        ...
        DOVirtual.Float(originDistorsion, endDistorsion, .5f, Distortion);
